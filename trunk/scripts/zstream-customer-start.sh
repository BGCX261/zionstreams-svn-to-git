...
function zstream_generate_customer_vlm_conf()
{
	...
	local address=$(zstream_conf_get_value $1 customer.server.address ; echo $retval)
	local port=$(zstream_conf_get_value $1 customer.server.port ; echo $retval)
	...
	do
		...
		do
			...
			echo "# " >> $vlmconf
			echo "new      "$vidsrc"-"$stream" broadcast enabled loop" >> $vlmconf
			echo "setup    "$vidsrc"-"$stream" input "$input_url >> $vlmconf
			echo "setup    "$vidsrc"-"$stream" output #std{access=http,mux=asf,dst="$base_url"/"$vidsrc/$stream"}" >> $vlmconf
			echo "setup    "$vidsrc"-"$stream" option http-reconnect" >> $vlmconf 
			echo "control  "$vidsrc"-"$stream" play" >> $vlmconf
...
ZSTREAM_CMDLINE=$VLC" -vvv -I telnet --vlm-conf "$ZSTREAM_CUSTOMER_VLM_CONF" --file-logging --logfile="$ZSTREAM_LOGFILE
